...
from qos_performance.util import ArgParser,keyval_int, FuncAction,token_split, Update,\
    keyval
from qos_performance import aggregators, loggers,resultset, formatters
from qos_performance.util import  token_split, clean_path, format_date
from qos_performance.settings import parser as SETTINGS_PARSER
from qos_performance.settings import _LOG_DEFER
from qos_performance.metadata import record_metadata, record_postrun_metadata
from qos_performance.aggregators import Aggregator
...
try :
    ...
    print("Please try : sudo pip3 install paramiko or sudo pip install paramiko if you're using python 2.x")
    print("You can try also sudo apt-get install python-paramiko")
...
class BatchRunner(object):
    ...
    def __init__(self, settings):
        ...
        self.ssh_client.connect(hostname=self.settings.HOST,username=self.settings.USER,password=self.settings.PASSWORD)'''
        settings.BATCH_FILES=[]
    ...
    def run_test(self, settings, output_path, print_datafile_loc=False):
        ...
        print("before settings.loadtest")
        settings.load_test()
        print("after settings.loadtest")
        ...
        print(" end dumpdir")
        '''logger.log(
                   "Data file written too %s.", output_path)
        logger.debug("Data file written to %s",output_path)'''
        ...
        logger.error("file written")
        logger.debug("Executing test  %s", output_path)
    ...
    def run(self):
        print(" THIS IS THE VALUE OF INPUT")
        print(self.settings.INPUT)
        print(" THIS IS THE VALUE OF BATCH_NAMES")
        print(self.settings.BATCH_NAMES)
    ...
    def run_run(self):
        if self.settings.TEST_NAME == "install-tools-on-remote-host":
            self.run_install_tools()
            #self.check_installed_tools()
        if self.settings.TEST_NAME == "check-installed-tools":
            self.check_installed_tools()              
                    
    ## run install tools (first thing to do in the remote host in order to run tests)        
    def run_install_tools(self):
        ## verify internet connection in the remote host
        stdin,stdout,stderr = self.execute_script_on_remote_host("install_tools.sh")
        print(stdout.readlines(),stderr.readlines())
        str1 = ''.join(stdout.readlines())
        print(str1)
        
    ## run a script to check if these tools are installed (iperf,netperf,curl,nuttcp,tcpdump)              
    def check_installed_tools(self):
        stdin,stdout,stderr = self.execute_script_on_remote_host("chech_installed_tools.sh")
    ...
    ## execute a script in a remote host with sudo privileges   
    def execute_script_on_remote_host(self,filename):
        ftp_client=self.ssh_client.open_sftp()
        ftp_client.put("scripts/"+filename,filename)
        ftp_client.close() 
        stdin,stdout,stderr=self.ssh_client.exec_command("sh "+filename)
        self.ssh_client.exec_command("rm "+filename)
