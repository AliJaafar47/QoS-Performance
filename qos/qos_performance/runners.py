...
from qos_performance import util, transformers
from qos_performance.build_info import DATA_DIR
from qos_performance.util import classname, ENCODING, Glob
...
class ProcessRunner(RunnerBase, threading.Thread):
    ...
    def run(self):
        ...
        if self.settings.INDEX == "1" or self.settings.INDEX == "0" :
            ...
            logger.debug("%s %s finished", self.__class__.__name__,
                         self.name, extra={'runner': self})
        elif self.settings.INDEX == "2" :
            print("*************************************")
            lene = len (self.settings.config)
                    
            cmd="nuttcp -S &"
            for i in  range(lene):                        
                protocol=self.settings.config[i]["protocol"]
                port=self.settings.config[i]["port"]
                buffer_length=self.settings.config[i]["buffer_length"]
                nbstreams=self.settings.config[i]["number_of_streams"]
                bandwidth=self.settings.config[i]["bandwidth"]
                print(port,buffer_length,nbstreams,bandwidth)
                cmd +=" nuttcp -p{0} -l {1} {2} -i{3} -T3 -R {4} -t -N {5}  127.0.0.1 & ".format(port,buffer_length,"-u",1,'100K',nbstreams)
        ...
        for line in lines:
            for regexp in cls.regexes:
                ...
                if match:
                    ...
                    if 'FirstSynAckTime' in rw:
                        result.append([rw['time'], rw['PageLoadingTime']])
                        
                    if 'DURATION' in rw:
                        #result.append([rw['time'], rw['adresseSource']])
...
class UserestimationRunner(RegexpRunner):
    ...
    regexes = [re.compile( r'^(?P<time>[0-9]+.[0-9]+) '
                           r'(?P<DNSLOOKupTime>[0-9]+.[0-9]+) '
                           r'(?P<FirstSynAckTime>[0-9]+.[0-9]+) '
                           r'(?P<PageLoadingTime>[0-9]+.[0-9]+)'
                          )]

    def __init__(self, config, INDEX, host, **kwargs):
        self.host = host
       
       
        super(UserestimationRunner, self).__init__(**kwargs)
    ...
    def check(self):
        ...
        args.setdefault('config', self.settings.config)
        ...
        #i = i + 1
        print(self.command)   
        super(UserestimationRunner, self).check()
        
    def find_binary(self, config, INDEX, length, host, **kwargs):
        
        print("cocococ",config)
        print("INDEXX",INDEX)
        ...
        return "curl_loop {length} {host2}".format(
                binary=curl,
                length=5*length,
                vide='{}',
...
class PingRunner(RegexpRunner):
    ...
    '''regexes = [re.compile(r'^(?P<TIME> [^ ]+)(?x)'
                          r'(?P<DURATION>[ ^ ]\S*\s+).*'
                          r'(?P<adressSource>[0-9]+[0-9]+[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)'
                          r'(?:.*[+](?P<adresDEST>[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+).*)'
    ...
    def find_binary(self, ip_version, length, host,
               ...
               "--ipv6" not in str(out):
                ...
                return "{binary} {ipver} -D -p {interval:.0f} -c {count:.0f} " \
                    "-t {timeout} {marking} {local_bind} {host}".format(
                       ...
                       #niwar comment #interval=interval * 1000,  # fping expects interval in ms
                        # since there's no timeout parameter for fping,
                        # calculate a total number of pings to send
                        #niwar comment#count=length // interval + 1,
        ...
        if ping is not None:
            ...
            # message.
            '''proc = subprocess.Popen([ping,
                                     '-c','1000','-i','enp4s0'] + pingargs,
                                    stdout=subprocess.PIPE,
                                    stderr=subprocess.PIPE)'''
            proc = subprocess.Popen([ping,
                                     '-h'] + pingargs,
            ...
            if hasattr(err, 'decode'):
                    err = err.decode(ENCODING)
            return "{binary} -m -s {length:d} {marking} " \
                "{local_bind} {pingargs} -i {host}".format(
                    binary=ping,
...
class HttpGetterRunner(RegexpRunner):
    ...
    def check(self):
        ...
        [self.settings.HOST] =self.settings.HOSTS
        print("self.settings.HOST :",self.settings.HOSTS)
